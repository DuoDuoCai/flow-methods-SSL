...
class RealNVPBase(nn.Module):
    ...
    def nll_old(self,x,y=None,label_weight=1.):
        # print("I am calling parent class for NLL")
        z = self(x)
        logdet = self.logdet()
        z = z.reshape((z.shape[0], -1))
        prior_ll = self.prior.log_prob(z, y,label_weight=label_weight)
        # print("the prior of RealNVPBase is:", type(self.prior))
        # print(self.prior)
        # sys.exit()
        nll = -(prior_ll + logdet)
        return nll
    
    def nll(self, x, y=None, label_weight=1.0):
        """
        Negative-log-likelihood (loss) with Jacobian clamp.
        x : tensor [B, …]
        y : labels or None
        """
        z       = self(x)                              # forward through flow
        logdet  = self.logdet()                        # [B]
        # ---------- 1. restrict Jacobian term ----------
        logdet  = logdet.clamp_(-1e3, 1e3)

        # ---------- 2. prior log-prob ----------
        z_flat  = z.reshape(z.shape[0], -1)
        prior_ll = self.prior.log_prob(z_flat, y, label_weight=label_weight)

        # ---------- 3. combine & safe guard ----------
        nll = -(prior_ll + logdet)                     # [B]
        # nll = torch.nan_to_num(nll, nan=1e3, posinf=1e3, neginf=-1e3)
...
class RealNVPTabular(RealNVPBase):
    ...
    def __init__(self, in_dim=2, num_coupling_layers=6, hidden_dim=256, 
        ...
        print("initializing real NVP tabular,  my parent's init, my prior is :")
        print("in_dim:", in_dim)
